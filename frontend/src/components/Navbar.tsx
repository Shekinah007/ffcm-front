...
const Navbar = (): ReactElement => {
    ...
    return (
        <div className={`justify-center flex fixed w-screen flex-col md:flex-row  md:justify-between min-h-[40px] items-center z-20 px-3 md:p-2 duration-400 md:px-10 ${open ? "bg-white/60 text-black" : "bg-black"} text-white border-b-2 border-yellow-500 `}>
            ...
            <div className={`absolute top-50% left-3 w-11/12 flex flex-row items-center justify-between gap-2 duration-500 -translate-y-0 ${open && "-translate-y-36"} md:hidden`}>
                <p className="page underline"> HOME </p>
            ...
            <div className={`nav  h-0 overflow-hidden py-0 px-3 duration-300  ${open && 'h-48'} font-semibold md:p-3 md:py-3 md:h-10 flex flex-col justify-end md:items-center top-28 left-0 gap-3 w-screen md:static md:flex-row`} >
                ...
                <Link to={"/"} className="p-1 hover:text-yellow-500 duration-300 md:border-b-2 border-yellow-500 hover:border-white hover:rounded-lg">HOME</Link>
                <Link to={"/about"} className="p-1 hover:text-yellow-500 duration-300 md:border-b-2 border-yellow-500 hover:border-white hover:rounded-lg" >ABOUT</Link>
                ...
                <a href="#gallery" className="p-1 hover:text-yellow-500 duration-300 md:border-b-2 border-yellow-500 hover:border-white hover:rounded-lg">  WHO WE ARE</a>

            </div>
            <div>
                <span className={`menu-btn md:hidden text-yellow-500 self-center translate-y-6 duration-500 text-3xl border-2 ${!open && " -scale-y-100"} h-12 w-12 bg-black flex items-center justify-center rounded-full`} onClick={(): void => { setOpen(prev => !prev); console.log(open) }}>
                    ^

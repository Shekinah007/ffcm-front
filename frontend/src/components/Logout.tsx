...
const Logout = ({ setIsLoggedIn, setMainCurrentPage, setLogoutModal }: any) => {
    ...
    const logOut = () => {
        ...
        setIsLoggedIn(false)
        setMainCurrentPage("Home")
        navigate("/")
    }


    return (
        <div className="log-out entrance fixed left-0 top-0 card z-[100]">
            <div className="bg-black/80 top-0 left-0 h-screen w-screen absolute -z-[10]"></div>
            <div className="card flex flex-col gap-2 fixed-center p-4 bg-white z-[1000] w-screen md:w-[400px] rounded-lg items-center self-center justify-self-center justify-center bg-cover ">
                ...
                <div className=" flex flex-row gap-4  text-white p-2 rounded-md">
                    <button
                        className="card text-lg rounded-md p-2  bg-red-500"
